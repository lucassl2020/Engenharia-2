...
class TelaListaFornecedores(QWidget):
    def __init__(self, parent=None):
        super(TelaListaFornecedores, self).__init__(parent)
    ...
    def _create_widgets(self):
        self.realizar_venda_label = label(self, "Fornecedores", 0, 0, 600, 60)
    ...
    def adicionarItemTabela(self, linha, valores):      
        ...
        self.tabela.setItem(linha, 5, QTableWidgetItem(str(valores[5])))
        self.tabela.setItem(linha, 6, QTableWidgetItem(str(valores[6])))

    def botaoVoltar(self):
        event = {"codigo": 33, "descricao": "Botão VOLTAR da tela LISTA FORNECEDORES"}
        self.subject.notify(event)


if __name__ == '__main__':
    root = QApplication(sys.argv)
    app = TelaListaFornecedores()
    app.show()
    sys.exit(root.exec_())
